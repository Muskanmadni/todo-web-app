...
const ReminderForm = ({ onSubmit, onCancel, initialData }: ReminderFormProps) => {
  const [title, setTitle] = useState(initialData?.title || '');
  const [description, setDescription] = useState(initialData?.description || '');
  const [priority, setPriority] = useState<ReminderFormData['priority']>(initialData?.priority || 'medium');
  const [tags, setTags] = useState<string[]>(initialData?.tags || []);
  const [currentTag, setCurrentTag] = useState('');
  const [dueDate, setDueDate] = useState(initialData?.dueDate || '');
  const [reminderTime, setReminderTime] = useState(initialData?.reminderTime || '');
  const [reminderMethod, setReminderMethod] = useState<ReminderFormData['reminderMethod']>(initialData?.reminderMethod || 'push');
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            id="priority"
            value={priority}
            onChange={(e) => setPriority(e.target.value as ReminderFormData['priority'])}
            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
          >
        ...
        <div>
          <label htmlFor="reminderTime" className="block text-sm font-medium text-gray-700 mb-1 flex items-center">
        ...
        <div>
          <label htmlFor="reminderMethod" className="block text-sm font-medium text-gray-700 mb-1 flex items-center">
            <Bell className="h-4 w-4 mr-1" />
            Notification Method
          </label>
          <select
            ...
            onChange={(e) => setReminderMethod(e.target.value as ReminderFormData['reminderMethod'])}
            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
          >
